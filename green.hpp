...
static inline void plot_ldos(const device & d, const potential & phi, const unsigned N_grid = 1000) {
    ...
    vband(d.sg)  += -0.5 * d.E_g;
    vband(d.g)   += -0.5 * d.E_g;
    vband(d.dg)  += -0.5 * d.E_g;
    ...
    cband(d.sg)  += +0.5 * d.E_g;
    cband(d.g)   += +0.5 * d.E_g;
    cband(d.dg)  += +0.5 * d.E_g;
    ...
    unsigned N_s = std::round(d.N_sc + 0.5 * d.N_sox);
    arma::vec fermi_l(N_s);
    fermi_l.fill(d.F_sc + phi.s());
    arma::vec x_l = d.x(arma::span(0, N_s-1));
    gp.add(x_l, fermi_l);

    unsigned N_d = std::round(d.N_dc + 0.5 * d.N_dox);
